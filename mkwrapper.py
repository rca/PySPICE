...
def gen_wrapper(prototype, buffer):
    ...
    for param in prototype_obj.params:
        ...
        #debug("unparsed: %s" % param)
        #debug("parsed param: %s" % param_info)

        if param_info is None: continue

        debug(param_info)

        if param_info.is_array and not param_info.is_const:
            t_type = OUTPUT_TYPE
        elif param_info.is_const or param_info.is_const or \
                 not param_info.is_pointer:
            t_type = INPUT_TYPE
    ...
    # parse the outputs
    if len(output_list): buffer.write("\n  /* variables for outputs */")
    ...
    # parse the inputs
    if len(input_list): buffer.write("\n  /* variables for inputs */")
    ...
    t_list = []
    for output in output_list:
        if output.is_array or \
                output.type == "SpiceChar" or \
...
def parse_prototype(prototype):
    ...
    #sys.stderr.write('function_name: %s, type: %s, py_string: %s\n' % (prototype_obj.function_name, prototype_obj.type, prototype_obj.py_string))
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        if input == "":
            continue
        elif not parsing_prototype and input.find("(") == -1:
            continue
